...
parameter file (`set_parameters.R`). Keeping it that way enables reproducibilty by having one parameter set per experiment. Please have a set_parameter.R for further comments. You can find the example file in the repository https://github.com/Gaebler-Lab/Q4ddPCR/tree/Q4ddPCR_v1.1.0. Please use set_parameter.R, and CSV-file.csv & Excel-file.xlsx as data input files. Make sure the file paths in set_paramteres.R are adapted according to your directory structure.
...
if (!all(unique(dtQC$`Sample description 1`) %in% names(tar_mio_factor))) {
  ...
  warning(paste0("Set tar_mio_factor to 1 for samples ", setdiff(s_desc, names(tar_mio_factor))))
  tar_mio_factor[setdiff(s_desc, names(tar_mio_factor))] <- 1
}
```
...
```{r split-wells}
water_wells <- unique(dtQC[dtQC$`Sample description 1` %in% water_name ,"Well"])
shear_wells <- setdiff(unique(dtQC[dtQC$Target %in% shear_name,"Well"]), water_wells)
